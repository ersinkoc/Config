# Config - Zero-Dependency NPM Package

## Package Identity

| Field | Value |
|-------|-------|
| **NPM Package** | `@oxog/config` |
| **GitHub Repository** | `https://github.com/ersinkoc/config` |
| **Documentation Site** | `https://config.oxog.dev` |
| **License** | MIT |
...
**One-line:** Zero-dependency configuration loader with multi-format support, environment merging, and plugin-based extensibility

Config is a powerful, zero-dependency configuration management library for Node.js applications. It supports loading configurations from multiple file formats (JSON, YAML, TOML, INI, ENV, JS/TS), intelligently merges environment-specific overrides, and provides a micro-kernel architecture for extending functionality through plugins. Features include deep merging with customizable strategies, optional schema validation, file watching for hot reload, encrypted secrets support, and comprehensive TypeScript support.
...
Load configuration files from multiple formats with automatic detection based on file extension.

**Supported Formats:**
- **JSON** - `.json` files
- **YAML** - `.yaml`, `.yml` files (custom parser implementation)
- **TOML** - `.toml` files (custom parser implementation)
- **INI** - `.ini` files (custom parser implementation)
- **ENV** - `.env` files (custom parser implementation)
- **JavaScript** - `.js`, `.mjs` files (ESM import)
- **TypeScript** - `.ts`, `.mts` files (esbuild-free evaluation)
...
Automatically merge environment-specific configurations with smart resolution order.
...
Intelligent deep merge with configurable strategies for arrays, objects, and special values.
...
- `unique` - Merge arrays with unique values
...
Validate configuration against a schema with detailed error messages.
...
```typescript
/**
 * Plugin interface for extending config functionality.
 * 
...
| Plugin | Description |
|--------|-------------|
| `json-parser` | Parse JSON configuration files |
| `env-parser` | Parse .env files and environment variables |
| `merge` | Deep merge configurations with strategies |
| `defaults` | Apply default values to configuration |
...
### Kernel
- `loadConfig(options)` - Load config from files
- `createConfig(data)` - Create config from object
- `defineConfig(data)` - Define typed config
...
- `watch()` / `unwatch()` - File watching

### Core Plugins
- `json-parser` - JSON file support
- `env-parser` - ENV file/variable support
- `merge` - Deep merge with strategies
- `defaults` - Default value support

### Optional Plugins
- `yaml-parser` - YAML file support
- `toml-parser` - TOML file support
- `ini-parser` - INI file support
...
config.on('change', (e) => console.log('Changed:', e.path));
```
...
|------|---------|----------|
| `CONFIG_NOT_FOUND` | Config file not found | Check file path and cwd |
| `PARSE_ERROR` | Failed to parse file | Check file syntax |
| `VALIDATION_ERROR` | Schema validation failed | Fix config values |
| `REQUIRED_MISSING` | Required field missing | Add required fields |
| `ENCRYPTION_ERROR` | Decryption failed | Check encryption key |

## Links

- Docs: https://config.oxog.dev
- GitHub: https://github.com/ersinkoc/config
```
...
Write a complete YAML parser from scratch supporting:
...
Write a complete TOML parser from scratch supporting:
- Key-value pairs
...
### INI Parser (Custom Implementation)

Write a complete INI parser from scratch supporting:
- Key-value pairs
- Sections `[section]`
...
### ENV Parser (Custom Implementation)

Write a complete ENV parser from scratch supporting:
- Key-value pairs
...
Write a custom file watcher using Node.js `fs.watch` and `fs.watchFile`:
- Debounce rapid changes
- Handle file deletion and recreation
- Support recursive directory watching
- Platform-specific handling (FSEvents on macOS, inotify on Linux)
...
Implement encryption using Node.js `crypto` module:
...
```typescript
/**
 * Base error class for all config errors.
 */
...
```json
{
  "name": "@oxog/config",
  "version": "1.0.0",
  "description": "Zero-dependency configuration loader with multi-format support and plugin extensibility",
  "type": "module",
  "main": "./dist/index.cjs",
  ...
  "repository": {
    "type": "git",
    "url": "git+https://github.com/ersinkoc/config.git"
  },
  "bugs": {
    "url": "https://github.com/ersinkoc/config/issues"
  },
  "homepage": "https://config.oxog.dev",
  "engines": {
    "node": ">=18"
  },
  "devDependencies": {
...
- Custom parsers for YAML, TOML, INI, ENV (no external libraries!)
